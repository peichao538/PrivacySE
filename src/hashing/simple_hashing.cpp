...
void *gen_entries(void *ctx_tmp) {
	...
	// modified by peic 20220425
	inbytelen = ctx->hs->inbytelen;
	eleptr = ctx->elements + ctx->startpos * inbytelen;
...
void init_hash_table(sht_ctx* table, uint32_t nelements, hs_t* hs) {
    ...
    // Todo: use 1.2 as epsilon, so 'n' nearly equal to 'nbin', mybe 'maxbinsize' set as 32 is enough for hash failure 2**(-40)
	//table->maxbinsize = 32;
