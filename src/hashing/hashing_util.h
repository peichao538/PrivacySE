...
static const uint8_t BYTE_SELECT_BITS_MASK[8] = {0xFF, 0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3F, 0x7F};
...
inline void hashElement(uint8_t* element, uint32_t* address, uint8_t* val, hs_t* hs) {
	...
	*((uint32_t*) val)  = (uint32_t) R;
	...
	if(hs->inbitlen > sizeof(uint32_t) * 8) {
		...
		val[hs->outbytelen-1] &= (BYTE_SELECT_BITS_MASK[hs->outbitlen & 0x07]);
