...
static void CleanAESKey(AES_KEY_CTX* ctx, uint32_t numkeys, crypto* crypt)
{
	for(uint32_t i=0; i<numkeys; i++ )
	{
		crypt->clean_aes_key(ctx + i);
	}
}
...
class KKOTExtSnd {
  ...
  public:
	...
	~KKOTExtSnd(){
		m_nU.delCBitVector();
		CleanAESKey(m_vKeySeeds, m_nBaseOTs, m_cCrypto);
  ...
  public:
	...
	*/
	...
	~KKOTExtRcv() {
		...
		CleanAESKey(m_vKeySeedMtx, m_nBaseOTs * 2, m_cCrypto);
